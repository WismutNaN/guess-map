...
- [ ] Приложение запускается через `cargo tauri dev`
- [ ] Карта показывает мировую подложку с pan/zoom
- [ ] На карте видны полигоны ≥ 200 стран с границами
- [ ] На карте видны города с zoom-зависимой фильтрацией (столицы при zoom < 4, больше городов при zoom > 6)
- [ ] SQLite содержит ≥ 200 country records и ≥ 3000 admin1 records
- [ ] Позиция карты сохраняется между перезапусками
- [ ] Все тесты проходят
...
- [ ] На карте видна заливка driving_side (left=синий, right=красный) для всех стран
- [ ] На карте видны флаги стран при достаточном зуме
- [ ] Layer Panel показывает ≥ 5 типов подсказок с checkbox
- [ ] Включение/выключение слоя мгновенно отражается на карте
- [ ] LayerCompiler корректно собирает GeoJSON (unit тесты)
- [ ] Все тесты проходят
