...
- **Отдельный слой на тип данных** — каждый повторяемый класс подсказок оформляется отдельным `hint_type` (не складируется в `note`)
...
| 3.5 | ✅ Tauri command: `region::search` — поиск по имени (LIKE) | `src-tauri/src/commands/regions/` |
...
| 3.9 | ✅ Загрузка изображений: file picker → копирование в assets → создание asset record | `src/components/AssetUpload.tsx`, `src-tauri/src/commands/asset/` |
...
- [x] Все тесты проходят (`cargo test` + `npm test` + `npm run build`)
...
Фаза **5 завершена**: multi-select/lasso, bulk create/delete, Change Log и фильтры LayerPanel
работают в текущей архитектуре (`hooks + components + tauri commands`).
Дополнительно в bulk-UI поддержана привязка изображений/иконок к выбранным регионам.

### Задачи

| # | Задача | Файлы/модули |
|---|--------|-------------|
| 5.1 | ✅ Multi-select: Ctrl+Click добавляет/убирает из выделения | `src/map/interaction.ts`, `src/hooks/useSelectionState.ts` |
| 5.2 | ✅ Lasso select: Shift+Drag прямоугольное выделение | `src/map/interaction.ts` |
| 5.3 | ✅ "Select all in country" — через фильтр по country_code | `src/components/inspector/SelectionActions.tsx`, `src-tauri/src/commands/regions/` |
| 5.4 | ✅ Bulk Actions панель: появляется при > 1 выбранных, назначение hint | `src/components/BulkActions.tsx` |
| 5.5 | ✅ Tauri command `hint::batch_create` — создать hint для списка region_id | `src-tauri/src/commands/hints/` |
| 5.6 | ✅ Tauri command `hint::batch_delete` — удалить hints по типу для списка region_id | `src-tauri/src/commands/hints/` |
| 5.7 | ✅ Change Log view: список revision_log с фильтрами | `src/components/ChangeLog.tsx`, `src-tauri/src/commands/revisions/` |
| 5.8 | ✅ Фильтры: by author (user/agent), by entity_type, by date | `src/components/ChangeLog.tsx`, `src/components/ChangeLog.test.tsx` |
| 5.9 | ✅ Фильтр "empty regions": показать регионы без подсказок данного типа | `src/components/LayerPanel.tsx`, `src-tauri/src/commands/hints/repository.rs` |
| 5.10 | ✅ Фильтр по confidence: ползунок минимальной уверенности | `src/components/LayerPanel.tsx`, `src/map/hintLayers.ts` |
...
| E2E | ⚠️ Ctrl+Click на 3 региона → "3 selected" в панели (ручной smoke) |
| E2E | ⚠️ Apply driving_side=left к 5 регионам → все 5 получают подсказку (ручной smoke) |
| E2E | ⚠️ Change Log показывает batch операцию (ручной smoke) |

Автотесты Phase 5 проходят: `cargo test`, `npm test`, `npm run build`.

### Gate Criteria

- [x] Multi-select работает (Ctrl+Click)
- [x] Bulk apply создаёт hints для всех выбранных регионов
- [x] Change Log отображает историю с фильтрами
- [x] Фильтр "empty regions" корректно показывает незаполненные регионы
- [x] Все тесты проходят
